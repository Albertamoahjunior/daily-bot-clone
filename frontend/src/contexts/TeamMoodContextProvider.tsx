...
import { TeamMood, AllMoods } from "@/types/Mood";
...
export const teamMoodContext = createContext<{
    ...
    allTeamMoods: AllMoods | undefined;
    allMoodResponses: MoodResponse[] | undefined;
    // setAllMembers: React.Dispatch<React.SetStateAction<Member[]>>;
    setTeamMood: React.Dispatch<React.SetStateAction<MoodData[] | undefined>>;
} | undefined>(undefined);


const TeamMoodContextProvider = ({children}: ITeamMoodContextProvider) => {
    const [allTeamMoods, setAllTeamMoods] = useState<AllMoods>({});
    
    const fetchAllTeamMoods = async () =>{
        const teamMoods: AllMoods = await moodService.getMoods();
    ...
    const emojis:Emoji[] = [
        {emojiId: "001",emoji: "Excited", icon: <Laugh className="h-10 w-10 hover:text-green-500 transition-colors duration-300" />},
        {emojiId: "002",emoji: "Smile", icon: <Smile className="w-10 h-10 text-yellow-400" aria-label="Happy Mood" />},
        {emojiId: "003",emoji: "Meh", icon: <Meh className="w-10 h-10 text-gray-500" aria-label="Neutral Mood" />},
        {emojiId: "004",emoji: "Angry", icon: <Angry className="h-10 w-10 hover:text-orange-500 transition-colors duration-300" />},
        {emojiId: "005",emoji: "Sad", icon: <Frown className="h-10 w-10 hover:text-red-500 transition-colors duration-300" />}
    ]
    ...
    return (
        <teamMoodContext.Provider value={{ allTeamMoods,  teamMood, setTeamMood, allMoods, allMoodResponses, allEmojis }}>
            {children}
